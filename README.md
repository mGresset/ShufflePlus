...
> **État du projet :** la branche `9.9.x` est une candidate de finalisation avant la v10. La version actuelle est **9.9.44**.
...
Depuis la v9.9.31, le flux recommandé n’utilise plus **Ouvrir les URL X-Callback**. Le raccourci génère un UUID, ouvre Shuffle+ avec `requestId` et interroge ensuite le serveur Railway avec **Obtenir le contenu de l’URL**.
...
npm.cmd run validate
git add -A
git commit -m "Release Shuffle+ v9.9.44"
...
3. vérifier que l’en-tête affiche **v9.9.44** ;
...
bootstrap-9.9.44.js        Chargement versionné et migration du runtime
startup-recovery-9.9.44.js Réparation avant le chargement principal
...
La v9.9.44 est validée avec :

- **365 tests applicatifs réussis** ;
- tests du serveur réussis ;
- 155 fichiers JavaScript contrôlés ;
...
- `V9.9.44_NOTES.md` : changements propres à la version actuelle ;
- `DEPLOIEMENT-V9.9.44.md` : procédure courte de publication ;
- `INSTALLATION-V9.9.44.txt` : rappel d’installation ;
...
- `V9.x.x_NOTES.md` : historique détaillé des versions précédentes.
