...
test("les métadonnées de distribution annoncent la v10.5.2", () => {
    assert.equal(versionSource, "10.5.2");
    assert.match(packageSource, /"version": "10\.5\.2"/);
    assert.match(indexSource, /shuffleplus-version" content="10\.5\.2/);
    assert.match(indexSource, /startup-recovery-10\.5\.2\.js/);
});
