...
test("les métadonnées de distribution annoncent la v10.7.0", () => {
    assert.equal(versionSource, "10.7.0");
    assert.match(packageSource, /"version": "10\.7\.0"/);
    assert.match(indexSource, /shuffleplus-version" content="10\.7\.0/);
    assert.match(indexSource, /startup-recovery-10\.7\.0\.js/);
});
