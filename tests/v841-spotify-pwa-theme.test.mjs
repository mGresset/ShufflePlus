...
test("la distribution active annonce Shuffle+ 9.8.0", () => {
    assert.equal(versionSource, "9.8.0");
    assert.match(packageSource, /"version": "9\.8\.0"/);
    assert.match(indexSource, /shuffleplus-version" content="9\.8\.0/);
    assert.match(indexSource, /startup-recovery-9\.8\.0\.js/);
    assert.match(appSource, /const APP_VERSION = "9\.8\.0"/);
    assert.match(workerSource, /shuffleplus-v9\.8\.0/);
});
