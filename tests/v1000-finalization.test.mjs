...
const [
    ...
    readFile(new URL("../startup-recovery-11.0.0.js", import.meta.url), "utf8"),
...
test("Shuffle+ V11 annonce une release stable cohérente", () => {
    assert.equal(version, "11.0.0");
    assert.match(indexSource, /shuffleplus-release-channel" content="stable"/);
    assert.match(indexSource, /bootstrap-11\.0\.0\.js/);
    assert.match(workerSource, /shuffleplus-v11\.0\.0/);
});

test("V11 ne présente plus les numéros historiques actifs des sous-fonctions", () => {
    for (const marker of [
        "Shuffle+ v6",
        "Shuffle+ 8",
...
test("l’accueil stable n’utilise plus les rendus historiques V8/V9", () => {
...
test("la protection anti-boucle iPhone reste active en V11", () => {
...
test("le mode conduite garde Aléatoire et ne réintroduit pas le bouton Spotify supprimé", () => {
    assert.match(appSource, /data-driving-control="shuffle"/);
    assert.doesNotMatch(appSource, /data-driving-control="spotify"/);
});


test("V11 conserve les gros rendus de réglages hors de app.js", () => {
