...
const bootstrapSource = await readFile(new URL("../bootstrap-10.7.0.js", import.meta.url), "utf8");
const guardSource = await readFile(new URL("../update-guard.js", import.meta.url), "utf8");
const recoverySource = await readFile(new URL("../startup-recovery-10.7.0.js", import.meta.url), "utf8");
...
test("Shuffle+ 10.7.0 crée une transaction avant activation PWA", () => {
    ...
    assert.equal(transaction.toBuild, "10.7.0-pwa-reset-1");
    assert.equal(readPwaUpdateTransaction(storage).toVersion, "10.7.0");
...
test("update-guard surveille le démarrage et revient au cache précédent si nécessaire", () => {
    assert.ok(indexSource.indexOf("./update-guard.js") < indexSource.indexOf("./bootstrap-10.7.0.js"));
