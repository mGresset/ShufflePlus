...
test("la v9.7.2 charge le design system après les styles historiques", () => {
    assert.equal(versionSource, "9.7.2");

    const legacyStyle = indexSource.indexOf("style.css?v=9.7.2");
    const designStyle = indexSource.indexOf("design-system.css?v=9.7.2");

    assert.ok(legacyStyle > 0);
    assert.ok(designStyle > legacyStyle);
    assert.match(workerSource, /design-system\.css\?v=9\.7\.2/);
