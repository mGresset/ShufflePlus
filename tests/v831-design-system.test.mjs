...
test("la v9.9.1 charge le design system après les styles historiques", () => {
    assert.equal(versionSource, "9.9.1");

    const legacyStyle = indexSource.indexOf("style.css?v=9.9.1");
    const designStyle = indexSource.indexOf("design-system.css?v=9.9.1");

    assert.ok(legacyStyle > 0);
    assert.ok(designStyle > legacyStyle);
    assert.match(workerSource, /design-system\.css\?v=9\.9\.1/);
