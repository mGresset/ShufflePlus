...
test("la v8.8.0 charge le design system après les styles historiques", () => {
    assert.equal(versionSource, "8.8.0");

    const legacyStyle = indexSource.indexOf("style.css?v=8.8.0");
    const designStyle = indexSource.indexOf("design-system.css?v=8.8.0");

    assert.ok(legacyStyle > 0);
    assert.ok(designStyle > legacyStyle);
    assert.match(workerSource, /design-system\.css\?v=8\.8\.0/);
