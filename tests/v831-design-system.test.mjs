...
test("la v10.1.1 charge le design system après les styles historiques", () => {
    assert.equal(versionSource, "10.1.1");

    const legacyStyle = indexSource.indexOf("style.css?v=10.1.1");
    const designStyle = indexSource.indexOf("design-system.css?v=10.1.1");

    assert.ok(legacyStyle > 0);
    assert.ok(designStyle > legacyStyle);
    assert.match(workerSource, /design-system\.css\?v=10\.1\.1/);
