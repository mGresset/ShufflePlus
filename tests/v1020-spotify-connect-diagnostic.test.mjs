...
test("Shuffle+ 10.6.0 diagnostique le device actif même si /devices est vide", () => {
...
test("le marqueur d'autodiagnostic post-mise-à-jour est relisible et effaçable", () => {
    ...
    const marker = buildPostUpdateDiagnosticMarker({
        ...
        toBuild: "10.6.0-pwa-reset-1",
        createdAt: 1234
    });
    storage.setItem(POST_UPDATE_DIAGNOSTIC_KEY, JSON.stringify(marker));

    assert.equal(readPostUpdateDiagnosticMarker(storage).toBuild, "10.6.0-pwa-reset-1");
