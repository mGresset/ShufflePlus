...
test("la distribution active annonce Shuffle+ 8.4.0", () => {
    assert.equal(versionSource, "8.4.0");
    assert.match(indexSource, /shuffleplus-version" content="8\.4\.0/);
    assert.match(appSource, /const APP_VERSION = "8\.4\.0"/);
    assert.match(serviceWorkerSource, /shuffleplus-v8\.4\.0/);
});
